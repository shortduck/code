    	...
    	// Method called to display the expense details List
    	    function displayExpenseList(){
            		...
            		dojo.byId("cbSave").disabled = dojo.byId('saveBtn').disabled;
    	}
    	...
    	function updateResultGrid(data){
    		...
    		driverReimbursement = firstRow.driverReimbursement;
    		...
    		dojo.xhrPost({
    					...
    					load: function(data,args){
    						...
    						//if errors, do not pursue the effect of call!
    			    			...
    			    			setupDisplayCb(firstRow,true);//cn
    						}
    				}
    			});
    	...
    	function getExpenseDetailsCodingBlock(selectedRow){
    		...
    		driverReimbursement = row.driverReimbursement;
    	...
    	function submitCodingBlockUsingAjax(){
    		...
    		// Method to show checkboxes on the the Update column	
        	  ...
        	  function setupDisplayCb (rowItem,firstRow){
      			disableForm = dojo.byId('saveBtn').disabled;
    			if (disableForm){
    				// disable form fields
    				disableFields()
    				// disable checkboxes in each CB row
    				if (rowItem.chkBoxUpdate[0].indexOf('disabled') < 0){
    					disableCheckBoxes (rowItem);    	
    				}
    			} else if (rowItem.driverReimbursement == "Y"){
    				// disable form fields
    				disableFields();
    				// disable only the checkboxes for the expense detail line for Driver reimbursements only
    				if (rowItem.chkBoxUpdate[0].indexOf('disabled') < 0){
    					disableCheckBoxesDriverReimb(rowItem);
    				}
    			}
        	  }
        	...
        	// toggles only dojo combo boxes
        	  function toggleDojoControls (form, flag){
        	      ...
        	      if (dijitElement != undefined){
        	          if ((dijitElement.declaredClass == 'dijit.form.ComboBox') || 
        	        		  (dijitElement.declaredClass == 'dijit.form.FilteringSelect')){
        	  ...
        	  function showStatusMessage(messageText) {
        		...
        		function disableCheckBoxes(rowItem){
        		...
        		var grid = dijit.byId('resultCodingBlockGrid');
              	  grid.store.fetch({
          		    ...
          		    onComplete: function(items, request){                
          		    	dojo.forEach(items, function(item){
          		    		var cbStr = item.chkBoxUpdate[0];
                         		cbStr = cbStr.substring(0,cbStr.length-1);
                         		if (cbStr.indexOf('disabled') < 0){
                         			cbStr += ' disabled />';
                         		}
        		...
        		function disableCheckBoxesDriverReimb(rowItem){
        			// disable top level checkbox
        			if (!dojo.byId('chkBoxSelectAll').disabled)
        				dojo.byId('chkBoxSelectAll').disabled = true;
        			// disable checkboxes for expense line items
        			  var grid = dijit.byId('resultCodingBlockGrid');
                  	  grid.store.fetch({
