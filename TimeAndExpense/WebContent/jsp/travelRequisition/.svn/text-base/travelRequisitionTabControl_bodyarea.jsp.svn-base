...
<script type="text/javascript" src="${pageContext.request.contextPath}/js/expense_common.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/js/travelReqTripId.js"></script>

<script type="text/javascript" src="${pageContext.request.contextPath}/js/TravelRequisitionHistory.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/js/codingBlockCommon.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/js/TravelRequisitionCodingBlock.js"></script>	
<script type="text/javascript" src="${pageContext.request.contextPath}/js/TravelRequisitionCodingBlockExtended.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/js/codingBlockValidation.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/js/multipleAppointments.js"></script>
...
<script type="text/javascript"><!--
	...
	dojo.addOnLoad(function(){
		initSingleDateCalendar("f_requestDateField", "f_requestDateTrigger");
	...
	function summaryOperationPostProcessing(){
		...
		if(lastSuccessfulOperation != null && lastSuccessfulOperation.length > 0){
			var tab = dijit.byId('expense_tab_container');
			if(tab != null) tab.selectChild('summaryTab');

			// clear last action from session
			dojo.xhrPost({url: "ClearLastAction.action"});
		}
	}
...
// 			ddExpenseType_cb.store = expenseTypeCBStore;
	...
	function performTabLoadPostProcessing(){
		...
		dojo.connect(dijit.byId('travel_req_tab_container'), "selectChild", function(child){
			
			var result = null;
			var responseToConfirm = null;
			result = checkFormState();
			var tab = dijit.byId('travel_req_tab_container');
			var problemDuringSave = false;
			
			if(child.id == 'expenseDetailsTab'){				
				serverRequestExpenseDetailsInquiry("FindExpenseDetails.action", null, travelTypeChange);
			}
			...
			if ((result) && (result.indexOf("There are unsaved changes")) != -1
				...
				if(responseToConfirm)
				{
					...
					problemDuringSave = previousTab == "idTab"?saveTravelRequisition():(previousTab == "expenseDetailsTab"?saveExpenseDetails(): (previousTab == "liquidationsTab"?saveExpenseLiquidations():""));
				}
			...
			if(expenseReloadRequired != null && expenseReloadRequired){
				// reload expense
				window.location = window.location;
			}

			// reload tabs if required
			if(child.id == 'idTab' && expenseReloadRequired != null && expenseReloadRequired){
				// reload expense
				window.location = window.location;
			}

			// if expense details tab is selected
			else if(child.id == 'expenseDetailsTab' && child.isLoaded){
				var expDetailsGrid = dijit.byId('expenseDetailsGrid');
			 	if(expDetailsGrid != null){
			 		// Grid doesn't render in IE8 in hidden mode. So, just trigger grid rendering.
			 		expDetailsGrid.setStore(expDetailsGrid.store);
			 		...
			 		if(expDetailsGrid.rowCount < 1 && dojo.byId('fromDate').value.length < 1){
		...
		if (findElementPositionInArray(tabsToReload, 'summaryTab')<0) {
			tabsToReload.push('summaryTab');
		}

		// reload tab if required
		if(tabsToReload.length > 0){
			var ar_index = findElementPositionInArray(tabsToReload, child.id);
	...
	function initTravelRequisitionHistory(){
		displayTravelRequisitionHistoryList();
	}
	...
	function addTabPrefElement(formId, tabId){
		...
		function openSummaryReportUrl(){
			...
			winSummary = window.open(url, "PrintSummaryReport", "width=900px,height=650px,modal=no,scrollbars=1,resizable=yes,status=1");
			if (closed)
		...
		function openReportUrl(){
			...
			var url = "/TimeAndExpense/OpenTravelRequisitionReport.action";
			
			var closed = true;
			if (winDetail != undefined)
				closed = winDetail.closed;
			winDetail = window.open(url, "PrintTravelRequisitionReport", "width=900px,height=650px,modal=no,scrollbars=1,resizable=yes,status=1");
			if (closed)
...
<label for="treqEventId">Request Id:</label>
<input type="text" id="treqEventId" size="10" style="border: none"
	readonly="readonly">
<jsp:include page="../empHeaderInfo.jsp" />
<div style="height: 5px">
	<!-- -->
</div>
<div id="travel_req_tab_container" dojotype="dijit.layout.TabContainer" controllerWidget="dijit.layout.TabController"
	style="width: 98%; height:590px; position: relative;">
	<div id="idTab" dojotype="dijit.layout.ContentPane" title="ID">
		<span><jsp:include page="travelRequisitionIdTab.jsp" /></span>
	</div>
	<div id="cbTab" dojotype="dijit.layout.ContentPane"
		...
		href="jsp/travelRequisition/travelRequisitionCodingBlock.jsp"
		onload="initCodingBlock();" preventCache="true" refreshOnShow="true"></div>
	<div id="historyTab" dojotype="dijit.layout.ContentPane"
		title="History" loadingMessage="Loading History. Please wait.."
		href="TravelRequisitionHistoryList.action"
		onload="initTravelRequisitionHistory();" preventCache="true" refreshOnShow="true"></div>
</div>
