...
import gov.michigan.dit.timeexpense.listener.ApplicationContextListener;
...
public class JRCHelperSample {
	...
	/**
	 * Logs on to all existing datasource using properties defined in JNDI
	 * and drops the scema/owner name from the table name in the report SQL.
	 * 
	 * @param clientDoc The reportClientDocument representing the report being used
	 * @throws ReportSDKException
	 */
	public static void logonDataSourceJndi(ReportClientDocument clientDoc) throws ReportSDKException,
	TimeAndExpenseException {
		String userId = "";
		String password = "";
		String dbUrl = "";
		String dbDriver = "";
		...
		try{
			...
			Context initCtx = new InitialContext();			
		  	userId = (String) initCtx.lookup("DB_USER_ID");
		  	password = (String) initCtx.lookup("DB_PWD");

		  	dbUrl = (String) initCtx.lookup("DB_CONN_URL");
		  	dbDriver = (String) initCtx.lookup("DB_DRIVER");
		  	dbJndiName = (String) initCtx.lookup("DB_JNDI_NAME");
		  	
		  	if (StringUtils.isEmpty(userId) ||
		  			StringUtils.isEmpty(password) ||
		  			StringUtils.isEmpty(dbUrl) ||
		  			StringUtils.isEmpty(dbDriver) ||
		  			StringUtils.isEmpty(dbJndiName)){
		  				throw new TimeAndExpenseException("JNDI paramaters for Crystal reports not defined!");
		  			}
		}
		catch (NamingException e ){
			throw new TimeAndExpenseException("Error locating JNDI data source");
	}
		// change qualified name for report tables
		changeDataSource(clientDoc, userId, password, dbUrl, dbDriver, dbJndiName);
	}
	/**
	 * Logs on to existing datasource using properties defined in JNDI 
	 * and drops the schema/owner name from the table name in the report SQL.
	 * 
	 * @param clientDoc
	 * @param userId
	 * @param password
	 ...
	 */
	public static void logonDataSourceJndi(ReportClientDocument clientDoc, Object applicationCache) throws ReportSDKException,
	TimeAndExpenseException {	
		Map cache = (Map) applicationCache;
		CrystalReportsParms crystalParms = (CrystalReportsParms) cache.get(IConstants.CrystalReportsParms);
		  	if (StringUtils.isEmpty(crystalParms.getUser()) ||
		  			StringUtils.isEmpty(crystalParms.getPassword()) ||
		  			StringUtils.isEmpty(crystalParms.getConnectionUrl()) ||
		  			StringUtils.isEmpty(crystalParms.getDbDriver()) ||
		  			StringUtils.isEmpty(crystalParms.getJndiName())){
		  				throw new TimeAndExpenseException("JNDI paramaters for Crystal reports not defined!");
		  			}
			// change qualified name for report tables
		changeDataSource(clientDoc, crystalParms.getUser(), crystalParms.getPassword(), 
				crystalParms.getConnectionUrl(), crystalParms.getDbDriver(), crystalParms.getJndiName());
	}
	...
	public static CrystalReportsParms getCrystalReportParms () throws Exception{
		CrystalReportsParms cr = new CrystalReportsParms();
		try {
			Context initCtx = new InitialContext();
			String crCredAlias = (String) initCtx
					.lookup(IConstants.CrystalReportsAlias);
			if (StringUtils.isEmpty(crCredAlias)) {
				throw new Exception(
						"JNDI paramaters for Crystal reports alias not defined!");
			}
			SecurityConfig sec = SecurityConfig.getConfig();
			Properties prop = sec.getAuthData(crCredAlias);
			cr.setUser((prop.getProperty("username")));
			cr.setPassword((prop.getProperty("password")));
		 	cr.setConnectionUrl((String) initCtx.lookup("DB_CONN_URL"));
		  	cr.setDbDriver((String) initCtx.lookup("DB_DRIVER"));
		  	cr.setJndiName((String) initCtx.lookup("DB_JNDI_NAME"));
		  	cr.setReportsLocation((String) initCtx.lookup("REPORTS_LOCATION"));
		}catch (NameNotFoundException nnfe){
			log.error("Crystal report alias 'CrystalReportsAlias' not defined");
			throw nnfe;
		}catch (Exception e) {
			log.error("JNDI CRCred problem.", e);
			throw e;
		}
		return cr;
	}
